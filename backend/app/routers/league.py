...
def create_league(league: schemas.LeagueCreate, db: Session = Depends(get_db)):
    return crud.league.createLeague(db, league)
...
def get_leagues(db: Session = Depends(get_db)):
    return crud.league.getLeagues(db)
...
def update_league(
    ...
    if not league:
        raise HTTPException(status_code=404, detail="League not found")
    return crud.league.updateLeague(db, league_id, league_in)
...
def delete_league(league_id: int, db: Session = Depends(get_db)):
    ...
    if not league:
        raise HTTPException(status_code=404, detail="League not found")
    return crud.league.deleteLeague(db, league_id)
