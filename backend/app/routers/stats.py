...
from app.services.stats_sync_service import sync_and_save_season_stats, sync_and_save_team_stats
...
@router.get("/team/{team_id}/league/{league_id}/season/{season_id}", response_model=schemas.TeamSeasonStats)
async def get_team_stats_for_team_league_season(
    ...
    if not stats:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
            detail="Team stats introuvables"
        )
